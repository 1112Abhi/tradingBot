...
from core.candle_watcher import CandleWatcher, StrategyGroup, SYNC_INTERVALS
from strategy.ema_crossover import MACrossoverStrategy


def _make_watcher(symbol: str, interval: str, db: Database) -> CandleWatcher:
    """Build a minimal CandleWatcher with one portfolio group for tests."""
    group = StrategyGroup(
        symbol=symbol,
        interval=interval,
        mode="portfolio",
        strategies=[MACrossoverStrategy(rsi_buy_min=0, rsi_buy_max=100)],
        sl_pct=0.015,
        tp_pct=0.060,
    )
    return CandleWatcher(groups=[group], db=db)
...
class TestPhase76PollOnceFix(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.watcher = _make_watcher(self.symbol, self.interval, self.db)
    ...
    def test_poll_once_processes_only_bars_with_next_bar(self):
        ...
        # Mock _fetch_symbol to do nothing
        self.watcher._fetch_symbol = MagicMock()

        # Mock _process_candle to track calls
        processed_timestamps = []
        def track_process(g, ts, trade_type):
    ...
    def test_poll_once_with_single_bar_no_processing(self):
        ...
        processed_timestamps = []
        self.watcher._process_candle = lambda g, ts, trade_type: processed_timestamps.append(ts)
...
class TestPhase76MultiIntervalSync(unittest.TestCase):
    ...
    def test_fetch_with_retry_calls_loader_sync_for_all_intervals(self):
        """
        Verify _fetch_symbol calls loader.sync() for each interval in SYNC_INTERVALS.
        """
        sync_calls = []
        ...
        self.watcher.loaders[self.symbol].sync = mock_sync

        # Call the real _fetch_symbol
        self.watcher._fetch_symbol(self.symbol)
    ...
    def test_fetch_with_retry_returns_strategy_interval_count(self):
        """
        Verify _fetch_symbol syncs all intervals for the given symbol.
        """
        sync_calls = []
        def mock_sync(symbol, interval):
            sync_calls.append(interval)
            return 1

        self.watcher.loaders[self.symbol].sync = mock_sync
        self.watcher._fetch_symbol(self.symbol)

        self.assertEqual(set(sync_calls), set(SYNC_INTERVALS),
                         "Should sync all SYNC_INTERVALS")
